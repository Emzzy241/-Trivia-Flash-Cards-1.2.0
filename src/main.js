        ...
        $(".start-game").click(function () {
            ...
            function getTrivias(myTriviaResponse) {
                ...
                if (myTriviaResponse.results) {
                    ...
                    // returning an error message for my users
                    $("#question").text(`Sorry, there was an error in processing the Trivia Questions ${myTriviaResponse.message}`)
                }

            }

            // having written my function for successful or unsuccessful response
            // its time to make use of the class in business logic that helps with the API call

            TriviaService.getMeTriviaQuestions(userDifficulty)
            // no semi-colns at the end of this because of the .then() that's handling it later
            
            // a .then() to handle the resolved promise
            .then( function(gottenTrivias){
                // calling the function that handles my resolved promise
                getTrivias(gottenTrivias);
            })
